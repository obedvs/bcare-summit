...
const Services = () => {
  ...
  const [info, setInfo] = useState<Info>(infoOptions.pacientes);
    ...
    <section className="pt-12" id="services">
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-2 gap-8 container mx-auto py-10 px-2">
        ...
        <article className="w-full">
          <h2 className="text-2xl tracking-tight sm:text-4xl mb-8">{info.title}</h2>
          <p className="text-lg font-light leading-7 mb-8">{info.description}</p>
          <Link href="/#why" className="bg-[#562CB4] py-2 px-6 rounded-3xl font-medium text-white">
            Conoce más
          </Link>
        </article>
      </div>
